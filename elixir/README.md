...
uses only the lightweight `/v1/intake` gate. When configured control is unavailable, new dispatch and
retries fail closed, while already-running workers and reconciliation continue. The native Phoenix
page remains a read-only runtime diagnostics surface.
